_speedrunStatsHandler = function () {
	...
	*/
	...
	var onTransitionEnd = function (level, transitionTime) {
		...
		var levelLoadTime = (level <= 14 ? 3 : 4.5);
	...
	var displaySpeedrunStats = function () {
		...
		$("#total_dashes_stat").text(speedrun_stats.totalDashes);
		...
		for (var level = 1; level <= 15; level++) {
			...
			if (!currentLevelLoadTime || (level <= 14 && currentLevelLoadTime < 2.98) || (level == 15 && currentLevelLoadTime < 4.48)) {
				speedrun_stats.fake = true;
			}
		}
