    ...
    _createClass(Player, [{
      ...
      value: function applyAttack(attacker) {
        ...
        if (this.es > 0) {
          ...
          if (attacker.poi > 0) {
            this.addState(1, 'poison', {
              added: [['mhp', 'deg', attacker.poi / 100]]
            }, Infinity);
          }
        }
        ...
        if (attacker.wea > 0 && this.states.filter(function (s) {
          ...
          this.addState(13, 'weakness', {
            multis: [['dmg', 1 - attacker.wea / 100], ['aps', 1 - attacker.wea / 100]]
          }, Infinity);
        }
      }
    }, {
      ...
      value: function trigger(name) {
        ...
        if ((this[name] > 0 || args.isGoblet) && (this.used[name] < uses || !this.used[name])) {
          switch (name) {
            case 'res':
              this.hp += this[name] / 100 * this.mhp;
              break;

            case 'ala':
              this.addState(2, 'alacrity', {
                inc: [['aps', this[name] / 100]]
              }, 2);
              break;

            case 'irn':
              this.addState(3, 'iron', {
                multis: [['arm', this[name] / 100]]
              }, Infinity, false);
              break;

            case 'wcr':
              this.addState(4, 'warcry', {
                multis: [['dmg', this[name] / 100]]
              }, 3);
              break;

            case 'wil':
              this.addState(5, 'willpower', {
                stats: [['blo', this[name]]]
              }, Infinity, false);
              break;

            case 'ods':
              this.addState(6, 'odds', {
                multis: [['dmg', this[name] / 100]]
              }, Infinity, false);
              break;

            case 'vig':
              this.removeState('vigour');
              this.addState(11, 'vigour', {
                added: [['mhp', 'hpr', this[name] / 100]]
              }, 3);
              break;

            case 'atu':
              this.addState(14, 'attunement', {
                inc: [['dmg', this[name] / 100]]
              }, Infinity, false);
              break;

            case 'cau':
              this.addState(15, 'caution', {
                stats: [['arm', this[name]]]
              }, Infinity, false);
              break;

            case 'std':
              this.addState(16, 'stand', {
                inc: [['dmg', 0.15], ['arm', 0.30]]
              }, this[name]);
              break;

            case 'dod':
              this.addState(10, 'dodge', {}, Infinity, true);
              break;

            case 'rev':
              this.hp = this.mhp * this[name] / 100;
              args.monsters.forEach(function (m) {
                return m.freeze(2);
              });
              break;

            case 'buf':
              this.addState(8, 'buff', args.stats, args.duration);
              break;

            case 'spe':
              this.trigger('men');
              this.addState(7, 'speed', args.stats, args.duration);
              break;

            case 'hph':
              this.trigger('rar');
              this.trigger('ren');
              this.addState(9, 'heal', args.stats, args.duration);
              break;

            case 'men':
              this.removeState('poison');
              this.removeState('weakness');
              break;
      ...
      value: function triggerGoblets() {
        ...
        this.goblets.forEach(function (goblet, index) {
          if (goblet && _this6.hp <= _this6.mhp * 0.25 * (index + 1)) {
            _this6.trigger(goblet.mainStat.name, 1, {
              stats: goblet.getStateStats(_this6),
              duration: goblet.getDuration(_this6),
              isGoblet: true
            });
          }
        });
