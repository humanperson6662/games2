...
(function (Game) {
    ...
    Engine.System.onInit = function () {
        if (Engine.Data.useLocalStorage) {
            Engine.Data.setID("com", "noa", "minflip0", "0");
        }
        else {
            Engine.Data.setID("c", "n", "minflip0", "0");
        }
...
(function (Game) {
    ...
    Game.addPath("load", patfli);
    Game.addPath("load", patdea);
    Game.addPath("load", patgra);
    ...
    Game.addAction("configure", function () {
        ...
        Resources.bgms[0].volume = Resources.bgms[0].restoreVolume = 1.9 * volumeScale * bgmsca;
        Resources.bgms[0].loopStart = 1.665;
        Resources.bgms[0].loopEnd = 194.215;
        ...
        */
        ...
        Resources.audfli = new Engine.AudioPlayer(patfli);
        Resources.audfli.preserved = true;
        Resources.audfli.volume = Resources.audfli.restoreVolume = 3.0 * volumeScale * sfxsca;
        Game.sfxs.push(Resources.audfli);
        Resources.auddea = new Engine.AudioPlayer(patdea);
        Resources.auddea.preserved = true;
        Resources.auddea.volume = Resources.auddea.restoreVolume = 3.4 * volumeScale * sfxsca;
        Game.sfxs.push(Resources.auddea);
        Resources.audgra = new Engine.AudioPlayer(patgra);
        Resources.audgra.preserved = true;
        Resources.audgra.volume = Resources.audgra.restoreVolume = 3.9 * volumeScale * sfxsca;
        Game.sfxs.push(Resources.audgra);
        /*
    ...
    (function (Arrow) {
        ...
        var Instance = /** @class */ (function (_super) {
            ...
            function Instance(_df) {
                ...
                Arrow.ins[Arrow.cou] = _this;
                Arrow.def[Arrow.cou] = _df;
                Arrow.def[Arrow.cou].instance.y -= Game.SceneMap.instance.ySizeTile * 1;
                Arrow.condef(Arrow.cou);
                Arrow.conspr(Arrow.cou);
                Arrow.conbox(Arrow.cou);
                Arrow.conani(Arrow.cou);
                if (Arrow.cou == 0)
                    Engine.System.addListenersFrom(Game.Arrow);
                _this.ind = Arrow.cou++;
                return _this;
            }
            return Instance;
        }(Game.Entity));
        Arrow.Instance = Instance;
        function onReset() {
            Arrow.resani();
        }
        Arrow.onReset = onReset;
        function onStepUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
            Arrow.steani();
        }
        Arrow.onStepUpdate = onStepUpdate;
        function onDrawObjects() {
            Arrow.draspr();
            Arrow.drabox();
        }
        Arrow.onDrawObjects = onDrawObjects;
        function onClearScene() {
            Arrow.cou = 0;
        }
        Arrow.onClearScene = onClearScene;
    })(Arrow = Game.Arrow || (Game.Arrow = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var Arrow;
    (function (Arrow) {
        ...
        var ena = [];
        function conani(i) {
            horfra[i] = Arrow.ins[i].getProperty("horfra");
            verfra[i] = Arrow.ins[i].getProperty("verfra");
            hordir[i] = Arrow.ins[i].getProperty("horanidir");
            verdir[i] = Arrow.ins[i].getProperty("veranidir");
            maxfra[i] = Arrow.ins[i].getProperty("maxfra");
        }
        Arrow.conani = conani;
        function resani() {
            for (var i = 0; i < Arrow.cou; i++) {
                ena[i] = Arrow.ins[i].getProperty("ena");
                stecou[i] = 0;
                ind[i] = ena[i] ? 0 : 1;
                setfra(i);
            }
        ...
        Arrow.resani = resani;
        function setfra(i) {
            Arrow.spr[i].setFull(true, false, Game.Resources.texgam, Game.SceneMap.instance.xSizeTile, Game.SceneMap.instance.ySizeTile, 0, 0, horfra[i] + hordir[i] * ind[i] * Game.SceneMap.instance.xSizeTile * 2, verfra[i] + verdir[i] * ind[i] * Game.SceneMap.instance.ySizeTile * 2, Game.SceneMap.instance.xSizeTile, Game.SceneMap.instance.ySizeTile);
        }
        function enaani(i) {
            ena[i] = true;
            ind[i] = 1;
...
(function (Game) {
    ...
    (function (Arrow) {
        Arrow.box = [];
        function conbox(i) {
            Arrow.box[i] = new Engine.Box();
            Arrow.box[i].enabled = Arrow.box[i].renderable = true;
            Arrow.box[i].x = Arrow.def[i].instance.x;
            Arrow.box[i].y = Arrow.def[i].instance.y;
            Arrow.box[i].xSize = Game.SceneMap.instance.xSizeTile;
            Arrow.box[i].ySize = Game.SceneMap.instance.ySizeTile;
            Arrow.box[i].data = i;
            Game.SceneMap.instance.boxesSolids.push(Arrow.box[i]);
        }
        Arrow.conbox = conbox;
        function drabox() {
            for (var i = 0; i < Arrow.cou; i++)
...
(function (Game) {
    ...
    (function (Arrow) {
        ...
        Arrow.dir = [];
        function condef(i) {
            Arrow.hor[i] = Arrow.ins[i].getProperty("hor");
            Arrow.dir[i] = Arrow.ins[i].getProperty("dir");
        }
        Arrow.condef = condef;
    })(Arrow = Game.Arrow || (Game.Arrow = {}));
})(Game || (Game = {}));
...
(function (Game) {
    ...
    (function (Arrow) {
        Arrow.spr = [];
        function conspr(i) {
            Arrow.spr[i] = new Engine.Sprite();
            Arrow.spr[i].enabled = true;
            Arrow.spr[i].x = Arrow.def[i].instance.x;
            Arrow.spr[i].y = Arrow.def[i].instance.y;
        }
        Arrow.conspr = conspr;
        function draspr() {
            for (var i = 0; i < Arrow.cou; i++)
...
(function (Game) {
    ...
    (function (Block) {
        ...
        var Instance = /** @class */ (function (_super) {
            ...
            function Instance(_df) {
                var _this = _super.call(this, _df) || this;
                Block.ins[Block.cou] = _this;
                Block.def[Block.cou] = _df;
                Block.def[Block.cou].instance.y -= Game.SceneMap.instance.ySizeTile * 1;
                Block.conspr(Block.cou);
                Block.conbox(Block.cou);
                Block.conmec(Block.cou);
                if (Block.cou == 0)
                    Engine.System.addListenersFrom(Game.Block);
                _this.ind = Block.cou++;
                return _this;
            }
            return Instance;
        ...
        Block.onReset = onReset;
        function onStepUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
        }
        Block.onStepUpdate = onStepUpdate;
        function onDrawObjects() {
            Block.draspr();
            Block.drabox();
        }
        Block.onDrawObjects = onDrawObjects;
        function onClearScene() {
            Block.cou = 0;
        }
        Block.onClearScene = onClearScene;
    })(Block = Game.Block || (Game.Block = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var Block;
    (function (Block) {
        Block.box = [];
        function conbox(i) {
            Block.box[i] = new Engine.Box();
            Block.box[i].renderable = true;
            Block.box[i].x = Block.def[i].instance.x;
            Block.box[i].y = Block.def[i].instance.y;
            Block.box[i].xSize = Game.SceneMap.instance.xSizeTile;
            Block.box[i].ySize = Game.SceneMap.instance.ySizeTile;
            Block.box[i].data = i;
            Game.SceneMap.instance.boxesSolids.push(Block.box[i]);
        }
        Block.conbox = conbox;
        function drabox() {
            for (var i = 0; i < Block.cou; i++)
                Block.box[i].render();
        }
        Block.drabox = drabox;
    })(Block = Game.Block || (Game.Block = {}));
...
(function (Game) {
    ...
    (function (Block) {
        var fra;
        Game.addAction("preconfigure", function () { return fra = Game.FrameSelector.complex("blo", Game.Resources.texgam, 200, 974); });
        var typ = [];
        var ena = [];
        function conmec(i) {
            typ[i] = Block.ins[i].getProperty("typ");
        }
        Block.conmec = conmec;
        function resmec() {
            for (var i = 0; i < Block.cou; i++) {
                ena[i] = !Block.ins[i].getProperty("ena");
                swi(i);
            }
        }
        Block.resmec = resmec;
        function swi(i) {
            ena[i] = !ena[i];
            Block.box[i].enabled = ena[i];
            fra[2 * typ[i] + (ena[i] ? 0 : 1)].applyToSprite(Block.spr[i]);
        }
        function onSwitchPressed() {
            for (var i = 0; i < Block.cou; i++) {
                if (typ[i] == Game.Switch.typ)
                    swi(i);
            }
        }
        Block.onSwitchPressed = onSwitchPressed;
    })(Block = Game.Block || (Game.Block = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (Block) {
        Block.spr = [];
        function conspr(i) {
            Block.spr[i] = new Engine.Sprite();
            Block.spr[i].enabled = true;
            Block.spr[i].x = Block.def[i].instance.x;
            Block.spr[i].y = Block.def[i].instance.y;
        }
        Block.conspr = conspr;
        function draspr() {
            for (var i = 0; i < Block.cou; i++)
...
(function (Game) {
    var CeilPatroller;
    (function (CeilPatroller) {
        CeilPatroller.preserved = false;
        CeilPatroller.ins = [];
        CeilPatroller.def = [];
        CeilPatroller.cou = 0;
        function onClearScene() { CeilPatroller.cou = 0; }
        CeilPatroller.onClearScene = onClearScene;
        var Instance = /** @class */ (function (_super) {
            __extends(Instance, _super);
            function Instance(_df) {
                var _this = _super.call(this, _df) || this;
                CeilPatroller.ins[CeilPatroller.cou] = _this;
                CeilPatroller.def[CeilPatroller.cou] = _df;
                CeilPatroller.def[CeilPatroller.cou].instance.x += Game.SceneMap.instance.xSizeTile * 0.5;
                CeilPatroller.def[CeilPatroller.cou].instance.y -= Game.SceneMap.instance.ySizeTile;
                CeilPatroller.conphy(CeilPatroller.cou);
                CeilPatroller.conspr(CeilPatroller.cou);
                CeilPatroller.conani(CeilPatroller.cou);
                CeilPatroller.conove(CeilPatroller.cou);
                CeilPatroller.consta(CeilPatroller.cou);
                if (CeilPatroller.cou == 0)
                    Engine.System.addListenersFrom(Game.CeilPatroller);
                _this.ind = CeilPatroller.cou++;
                return _this;
            }
            Instance.prototype.onSetFrame = function (_, __, fra) {
                CeilPatroller.fraani(this.ind, fra);
                CeilPatroller.fraove(this.ind, fra);
            };
            return Instance;
        }(Game.Entity));
        CeilPatroller.Instance = Instance;
        function onReset() {
            CeilPatroller.resphy();
            CeilPatroller.resspr();
            CeilPatroller.resove();
            CeilPatroller.respla();
            CeilPatroller.ressta();
        }
        CeilPatroller.onReset = onReset;
        function onMoveUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
            CeilPatroller.movphy();
            CeilPatroller.movpla();
            CeilPatroller.movove();
        }
        CeilPatroller.onMoveUpdate = onMoveUpdate;
        function onStepUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
            CeilPatroller.stesta();
            CeilPatroller.linsta();
        }
        CeilPatroller.onStepUpdate = onStepUpdate;
        function onTimeUpdate() {
            CeilPatroller.timpla();
            CeilPatroller.timphy();
            CeilPatroller.timspr();
            CeilPatroller.timove();
            CeilPatroller.timsta();
        }
        CeilPatroller.onTimeUpdate = onTimeUpdate;
        function onDrawObjects() {
            CeilPatroller.draspr();
            CeilPatroller.draphy();
            CeilPatroller.draove();
            CeilPatroller.drasta();
        }
        CeilPatroller.onDrawObjects = onDrawObjects;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var CeilPatroller;
    (function (CeilPatroller) {
        CeilPatroller.sta = [];
        function consta(i) {
            CeilPatroller.conwal(i);
        }
        CeilPatroller.consta = consta;
        function ressta() {
            CeilPatroller.reswal();
        }
        CeilPatroller.ressta = ressta;
        function stesta() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                switch (CeilPatroller.sta[i]) {
                    case CeilPatroller.stawal:
                        CeilPatroller.stewal(i);
                        break;
                }
            }
        }
        CeilPatroller.stesta = stesta;
        function linsta() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                switch (CeilPatroller.sta[i]) {
                    case CeilPatroller.stawal:
                        CeilPatroller.linwal(i);
                        break;
                    case CeilPatroller.stafal:
                        CeilPatroller.linfal(i);
                        break;
                }
            }
        }
        CeilPatroller.linsta = linsta;
        function timsta() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                switch (CeilPatroller.sta[i]) {
                    case CeilPatroller.stawal:
                        CeilPatroller.timwal(i);
                        break;
                }
            }
        }
        CeilPatroller.timsta = timsta;
        function drasta() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                switch (CeilPatroller.sta[i]) {
                    case CeilPatroller.stawal:
                        CeilPatroller.drawal(i);
                        break;
                }
            }
        }
        CeilPatroller.drasta = drasta;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (CeilPatroller) {
        CeilPatroller.ani = [];
        function conani(i) {
            CeilPatroller.ani[i] = new Utils.Animator();
            CeilPatroller.ani[i].owner = CeilPatroller.ani[i].listener = CeilPatroller.ins[i];
        }
        CeilPatroller.conani = conani;
        function fraani(i, fra) {
            fra.applyToSprite(CeilPatroller.spr[i]);
        }
        CeilPatroller.fraani = fraani;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (CeilPatroller) {
        ...
        CeilPatroller.conove = conove;
        function resove() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                ove[i].x = CeilPatroller.box[i].x;
                ove[i].y = CeilPatroller.box[i].y;
            }
        }
        CeilPatroller.resove = resove;
        function movove() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                ove[i].x = CeilPatroller.box[i].x;
                ove[i].y = CeilPatroller.box[i].y;
        ...
        CeilPatroller.movove = movove;
        function fraove(i, fra) {
            fra.applyToBox(ove[i]);
        }
        CeilPatroller.fraove = fraove;
        function timove() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                ove[i].x = CeilPatroller.ext[i].x;
                ove[i].y = CeilPatroller.ext[i].y;
            }
        }
        CeilPatroller.timove = timove;
        function draove() {
            for (var i = 0; i < CeilPatroller.cou; i++)
                ove[i].render();
        }
        CeilPatroller.draove = draove;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (CeilPatroller) {
        CeilPatroller.box = [];
        CeilPatroller.horvel = [];
        CeilPatroller.vervel = [];
        var maxvervel = 3;
        var gra = 0.1;
        CeilPatroller.grasca = [];
        CeilPatroller.gradir = [];
        CeilPatroller.horhit = [];
        CeilPatroller.verhit = [];
        CeilPatroller.horhitdir = [];
        CeilPatroller.verhitdir = [];
        CeilPatroller.ext = [];
        CeilPatroller.horextvel = [];
        CeilPatroller.verextvel = [];
        function conphy(i) {
            CeilPatroller.box[i] = new Engine.Box();
            CeilPatroller.box[i].enabled = CeilPatroller.box[i].renderable = true;
            CeilPatroller.fra[0].applyToBox(CeilPatroller.box[i]);
            Game.Platform.sol.push(CeilPatroller.box[i]);
        }
        CeilPatroller.conphy = conphy;
        function resphy() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                CeilPatroller.box[i].enabled = true;
                CeilPatroller.box[i].x = CeilPatroller.def[i].instance.x;
                CeilPatroller.box[i].y = CeilPatroller.def[i].instance.y;
                CeilPatroller.horvel[i] = 0;
                CeilPatroller.vervel[i] = 0;
                CeilPatroller.grasca[i] = 1;
                CeilPatroller.gradir[i] = -1;
                CeilPatroller.horhit[i] = null;
                CeilPatroller.verhit[i] = null;
                CeilPatroller.horhitdir[i] = 0;
                CeilPatroller.verhitdir[i] = 0;
                CeilPatroller.horextvel[i] = 0;
                CeilPatroller.verextvel[i] = 0;
            }
        }
        CeilPatroller.resphy = resphy;
        function movphy() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                CeilPatroller.horhit[i] = CeilPatroller.box[i].cast(Game.SceneMap.instance.boxesSolids, null, true, CeilPatroller.horvel[i], true, Engine.Box.LAYER_ALL);
                CeilPatroller.box[i].translate(CeilPatroller.horhit[i], true, CeilPatroller.horvel[i], true);
                CeilPatroller.horhitdir[i] = CeilPatroller.horhit[i] == null ? 0 : (CeilPatroller.horvel[i] < 0 ? -1 : 1);
                //horvel[i]=horhit[i]!=null?0:horvel[i];
                CeilPatroller.vervel[i] += gra * CeilPatroller.grasca[i] * CeilPatroller.gradir[i];
                CeilPatroller.vervel[i] = Math.abs(CeilPatroller.vervel[i]) > maxvervel ? Game.dir(CeilPatroller.vervel[i]) * maxvervel : CeilPatroller.vervel[i];
                CeilPatroller.verhit[i] = CeilPatroller.box[i].cast(Game.SceneMap.instance.boxesSolids, null, false, CeilPatroller.vervel[i] == 0 ? CeilPatroller.gradir[i] : CeilPatroller.vervel[i], CeilPatroller.vervel[i] != 0, Engine.Box.LAYER_ALL);
                CeilPatroller.box[i].translate(CeilPatroller.verhit[i], false, CeilPatroller.vervel[i], true);
                CeilPatroller.verhitdir[i] = CeilPatroller.verhit[i] == null ? 0 : (CeilPatroller.vervel[i] < 0 ? -1 : 1);
                CeilPatroller.vervel[i] = CeilPatroller.verhit[i] != null ? 0 : CeilPatroller.vervel[i];
            }
        }
        CeilPatroller.movphy = movphy;
        function timphy() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                CeilPatroller.horextvel[i] = (CeilPatroller.horextvel[i] + CeilPatroller.horvel[i]) * (Game.SceneFreezer.stoped ? 0 : 1);
                CeilPatroller.verextvel[i] = (CeilPatroller.verextvel[i] + CeilPatroller.vervel[i]) * (Game.SceneFreezer.stoped ? 0 : 1);
                CeilPatroller.ext[i] = CeilPatroller.box[i].getExtrapolation(Game.SceneMap.instance.boxesSolids, CeilPatroller.horextvel[i], CeilPatroller.verextvel[i], true);
                CeilPatroller.horextvel[i] = 0;
                CeilPatroller.verextvel[i] = 0;
            }
        }
        CeilPatroller.timphy = timphy;
        function draphy() {
            for (var i = 0; i < CeilPatroller.cou; i++)
                CeilPatroller.box[i].renderAt(CeilPatroller.ext[i].x, CeilPatroller.ext[i].y);
        }
        CeilPatroller.draphy = draphy;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (CeilPatroller) {
        ...
        CeilPatroller.respla = respla;
        function movpla() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                if (CeilPatroller.verhitdir[i] == CeilPatroller.gradir[i]) {
                    for (var _i = 0, _a = CeilPatroller.verhit[i]; _i < _a.length; _i++) {
                        var hit = _a[_i];
                        var nexpla = null;
                    ...
                    pla[i] = nexpla;
                    if (pla[i] != null) {
                        pla[i].chi.push(CeilPatroller.box[i]);
                    }
                }
                else
                    pla[i] = null;
        ...
        CeilPatroller.movpla = movpla;
        function timpla() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                if (pla[i] != null) {
                    CeilPatroller.horextvel[i] += pla[i].xvelmov() / Engine.Box.UNIT;
                    CeilPatroller.verextvel[i] += pla[i].yvelmov() / Engine.Box.UNIT;
                }
            }
        }
        CeilPatroller.timpla = timpla;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (CeilPatroller) {
        Game.addAction("preconfigure", function () {
            CeilPatroller.fra = Game.FrameSelector.complex("ceipat", Game.Resources.texgam, 56, 929);
        });
        CeilPatroller.spr = [];
        function conspr(i) {
            CeilPatroller.spr[i] = new Engine.Sprite();
            CeilPatroller.spr[i].enabled = true;
        }
        CeilPatroller.conspr = conspr;
        function resspr() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                CeilPatroller.spr[i].x = CeilPatroller.def[i].instance.x;
                CeilPatroller.spr[i].y = CeilPatroller.def[i].instance.y;
            }
        }
        CeilPatroller.resspr = resspr;
        function timspr() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                //spr[i].xMirror=horvel[i]==0?spr[i].xMirror:horvel[i]<0;
                CeilPatroller.spr[i].x = CeilPatroller.ext[i].x;
                CeilPatroller.spr[i].y = CeilPatroller.ext[i].y;
            }
        }
        CeilPatroller.timspr = timspr;
        function draspr() {
            for (var i = 0; i < CeilPatroller.cou; i++)
                CeilPatroller.spr[i].render();
        }
        CeilPatroller.draspr = draspr;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var CeilPatroller;
    (function (CeilPatroller) {
        CeilPatroller.stacou = CeilPatroller.stacou || 0;
        CeilPatroller.stafal = CeilPatroller.stacou++;
        var anifal;
        Game.addAction("configure", function () { anifal = new Utils.Animation("fal", false, CeilPatroller.fra, 5, [1, 2, 3, 4, 5, 6, 7, 8], null); });
        function setfal(i) {
            CeilPatroller.ani[i].setAnimation(anifal);
            CeilPatroller.sta[i] = CeilPatroller.stafal;
        }
        CeilPatroller.setfal = setfal;
        function linfal(i) {
            if (CeilPatroller.verhitdir[i] == CeilPatroller.gradir[i])
                CeilPatroller.setwal(i);
        }
        CeilPatroller.linfal = linfal;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (CeilPatroller) {
        CeilPatroller.stacou = CeilPatroller.stacou || 0;
        CeilPatroller.stawal = CeilPatroller.stacou++;
        var aniwal;
        Game.addAction("configure", function () { aniwal = new Utils.Animation("wal", true, CeilPatroller.fra, 7, [1, 2, 3, 4, 5, 6, 7, 8], null); });
        var vel = [];
        var patbox = [];
        var fliaby = [];
        function conwal(i) {
            vel[i] = CeilPatroller.ins[i].getProperty("vel");
            fliaby[i] = CeilPatroller.ins[i].getProperty("fliaby");
        ...
        CeilPatroller.conwal = conwal;
        function reswal() {
            for (var i = 0; i < CeilPatroller.cou; i++) {
                setwal(i);
            }
        }
        CeilPatroller.reswal = reswal;
        function setwal(i) {
            CeilPatroller.ani[i].setAnimation(aniwal);
            CeilPatroller.horvel[i] = vel[i] * (CeilPatroller.def[i].flip.x ? -1 : 1);
            CeilPatroller.sta[i] = CeilPatroller.stawal;
        }
        CeilPatroller.setwal = setwal;
        function stewal(i) {
            if (CeilPatroller.horhitdir[i] != 0 || (CeilPatroller.verhitdir[i] == CeilPatroller.gradir[i] && fliaby[i] && cheaby(i))) {
                //spr[i].xMirror=!spr[i].xMirror;
                CeilPatroller.horvel[i] *= -1;
            }
        }
        CeilPatroller.stewal = stewal;
        function cheaby(i) {
            patbox[i].x = CeilPatroller.box[i].x;
            patbox[i].y = CeilPatroller.box[i].y;
            return patbox[i].collide(Game.SceneMap.instance.boxesSolids) == null;
        }
        function linwal(i) {
            if (CeilPatroller.verhitdir[i] == 0)
                CeilPatroller.setfal(i);
        }
        CeilPatroller.linwal = linwal;
        function timwal(i) {
            patbox[i].x = CeilPatroller.ext[i].x;
            patbox[i].y = CeilPatroller.ext[i].y;
        }
        CeilPatroller.timwal = timwal;
        function drawal(i) {
            patbox[i].render();
        }
        CeilPatroller.drawal = drawal;
    })(CeilPatroller = Game.CeilPatroller || (Game.CeilPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (FloorPatroller) {
        ...
        var Instance = /** @class */ (function (_super) {
            ...
            function Instance(_df) {
                var _this = _super.call(this, _df) || this;
                FloorPatroller.ins[FloorPatroller.cou] = _this;
                ...
                _this.ind = FloorPatroller.cou++;
                return _this;
            }
            Instance.prototype.onSetFrame = function (_, __, fra) {
                FloorPatroller.fraani(this.ind, fra);
                FloorPatroller.fraove(this.ind, fra);
            };
            return Instance;
        }(Game.Entity));
        FloorPatroller.Instance = Instance;
        function onReset() {
...
(function (Game) {
    ...
    (function (FloorPatroller) {
        ...
        function conani(i) {
            ...
            FloorPatroller.ani[i].owner = FloorPatroller.ani[i].listener = FloorPatroller.ins[i];
        }
...
(function (Game) {
    ...
    (function (FloorPatroller) {
        Game.addAction("preconfigure", function () {
            FloorPatroller.fra = Game.FrameSelector.complex("flopat", Game.Resources.texgam, 0, 912);
        });
        FloorPatroller.spr = [];
        function conspr(i) {
...
(function (Game) {
    ...
    (function (FloorPatroller) {
        ...
        var fliaby = [];
        function conwal(i) {
            vel[i] = FloorPatroller.ins[i].getProperty("vel");
            fliaby[i] = FloorPatroller.ins[i].getProperty("fliaby");
            patbox[i] = new Engine.Box();
            patbox[i].enabled = patbox[i].renderable = true;
            patbox[i].xSize = patbox[i].ySize = 1;
            patbox[i].xOffset = -0.5;
...
(function (Game) {
    ...
    (function (FloorSwitch) {
        FloorSwitch.preserved = false;
        FloorSwitch.ins = [];
        FloorSwitch.def = [];
        FloorSwitch.cou = 0;
        function onClearScene() { FloorSwitch.cou = 0; }
        FloorSwitch.onClearScene = onClearScene;
        var Instance = /** @class */ (function (_super) {
            __extends(Instance, _super);
            function Instance(_df) {
                var _this = _super.call(this, _df) || this;
                FloorSwitch.ins[FloorSwitch.cou] = _this;
                FloorSwitch.def[FloorSwitch.cou] = _df;
                FloorSwitch.def[FloorSwitch.cou].instance.x += Game.SceneMap.instance.xSizeTile * 0.5;
                FloorSwitch.def[FloorSwitch.cou].instance.y -= (Game.SceneMap.instance.ySizeTile - FloorSwitch.fra[0].ySizeBox) * (FloorSwitch.def[FloorSwitch.cou].flip.y ? 1 : 0);
                FloorSwitch.conphy(FloorSwitch.cou);
                FloorSwitch.conspr(FloorSwitch.cou);
                FloorSwitch.conani(FloorSwitch.cou);
                FloorSwitch.conmec(FloorSwitch.cou);
                if (FloorSwitch.cou == 0)
                    Engine.System.addListenersFrom(Game.FloorSwitch);
                _this.ind = FloorSwitch.cou++;
                return _this;
            }
            Instance.prototype.onSetFrame = function (_, __, fra) {
                FloorSwitch.fraani(this.ind, fra);
            };
            return Instance;
        }(Game.Entity));
        FloorSwitch.Instance = Instance;
        function onReset() {
            FloorSwitch.resphy();
            FloorSwitch.resspr();
            FloorSwitch.resmec();
        ...
        FloorSwitch.onMoveUpdate = onMoveUpdate;
        function onOverlapUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
            FloorSwitch.ovemec();
        }
        FloorSwitch.onOverlapUpdate = onOverlapUpdate;
        function onTimeUpdate() {
            FloorSwitch.timpla();
            FloorSwitch.timphy();
            FloorSwitch.timspr();
        }
        FloorSwitch.onTimeUpdate = onTimeUpdate;
        function onDrawObjects() {
            FloorSwitch.draspr();
            FloorSwitch.draphy();
        }
        FloorSwitch.onDrawObjects = onDrawObjects;
    })(FloorSwitch = Game.FloorSwitch || (Game.FloorSwitch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var FloorSwitch;
    (function (FloorSwitch) {
        FloorSwitch.ani = [];
        function conani(i) {
            FloorSwitch.ani[i] = new Utils.Animator();
            FloorSwitch.ani[i].owner = FloorSwitch.ani[i].listener = FloorSwitch.ins[i];
        }
        FloorSwitch.conani = conani;
        function fraani(i, fra) {
            fra.applyToSprite(FloorSwitch.spr[i]);
            FloorSwitch.spr[i].yOffset += FloorSwitch.spr[i].yMirror ? FloorSwitch.box[i].ySize : 0;
        }
        FloorSwitch.fraani = fraani;
    })(FloorSwitch = Game.FloorSwitch || (Game.FloorSwitch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (FloorSwitch) {
        ...
        var anidow;
        Game.addAction("configure", function () {
            aninor = new Utils.Animation("nor", true, FloorSwitch.fra, 8, [0, 1, 2, 1], null);
            anidow = new Utils.Animation("dow", false, FloorSwitch.fra, 8, [3], null);
        });
        var typ = [];
        var dow = [];
        var offfra = 4;
        function conmec(i) {
            typ[i] = FloorSwitch.ins[i].getProperty("typ");
            FloorSwitch.ani[i].off = typ[i] * offfra;
        }
        FloorSwitch.conmec = conmec;
        function resmec() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                dow[i] = FloorSwitch.ins[i].getProperty("dow");
                FloorSwitch.ani[i].setAnimation(dow[i] ? anidow : aninor);
            }
        }
        FloorSwitch.resmec = resmec;
        function ovemec() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                if (!dow[i] && FloorSwitch.box[i].collideAgainst(Game.pla.box) != null) {
                    Game.Switch.typ = typ[i];
                    Engine.System.triggerCustomEvent("onSwitchPressed");
                }
            }
        }
        FloorSwitch.ovemec = ovemec;
        function onSwitchPressed() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                if (typ[i] == Game.Switch.typ) {
                    dow[i] = !dow[i];
        ...
        FloorSwitch.onSwitchPressed = onSwitchPressed;
    })(FloorSwitch = Game.FloorSwitch || (Game.FloorSwitch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (FloorSwitch) {
        ...
        FloorSwitch.horhit = [];
        FloorSwitch.verhit = [];
        FloorSwitch.horhitdir = [];
        FloorSwitch.verhitdir = [];
        FloorSwitch.ext = [];
        FloorSwitch.horextvel = [];
        FloorSwitch.verextvel = [];
        function conphy(i) {
            FloorSwitch.box[i] = new Engine.Box();
            FloorSwitch.box[i].enabled = FloorSwitch.box[i].renderable = true;
            FloorSwitch.fra[0].applyToBox(FloorSwitch.box[i]);
            Game.Platform.sol.push(FloorSwitch.box[i]);
        }
        FloorSwitch.conphy = conphy;
        function resphy() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                FloorSwitch.box[i].enabled = true;
                FloorSwitch.box[i].x = FloorSwitch.def[i].instance.x;
                FloorSwitch.box[i].y = FloorSwitch.def[i].instance.y;
                FloorSwitch.horvel[i] = 0;
                FloorSwitch.vervel[i] = 0;
                FloorSwitch.grasca[i] = 1;
                FloorSwitch.gradir[i] = FloorSwitch.def[i].flip.y ? -1 : 1;
                FloorSwitch.horhit[i] = null;
                FloorSwitch.verhit[i] = null;
                FloorSwitch.horhitdir[i] = 0;
                FloorSwitch.verhitdir[i] = 0;
                FloorSwitch.horextvel[i] = 0;
                FloorSwitch.verextvel[i] = 0;
            }
        }
        FloorSwitch.resphy = resphy;
        function movphy() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                FloorSwitch.horhit[i] = FloorSwitch.box[i].cast(Game.SceneMap.instance.boxesSolids, null, true, FloorSwitch.horvel[i], true, Engine.Box.LAYER_ALL);
                FloorSwitch.box[i].translate(FloorSwitch.horhit[i], true, FloorSwitch.horvel[i], true);
                FloorSwitch.horhitdir[i] = FloorSwitch.horhit[i] == null ? 0 : (FloorSwitch.horvel[i] < 0 ? -1 : 1);
                //horvel[i]=horhit[i]!=null?0:horvel[i];
                FloorSwitch.vervel[i] += gra * FloorSwitch.grasca[i] * FloorSwitch.gradir[i];
                FloorSwitch.vervel[i] = Math.abs(FloorSwitch.vervel[i]) > maxvervel ? Game.dir(FloorSwitch.vervel[i]) * maxvervel : FloorSwitch.vervel[i];
                FloorSwitch.verhit[i] = FloorSwitch.box[i].cast(Game.SceneMap.instance.boxesSolids, null, false, FloorSwitch.vervel[i] == 0 ? FloorSwitch.gradir[i] : FloorSwitch.vervel[i], FloorSwitch.vervel[i] != 0, Engine.Box.LAYER_ALL);
                FloorSwitch.box[i].translate(FloorSwitch.verhit[i], false, FloorSwitch.vervel[i], true);
                FloorSwitch.verhitdir[i] = FloorSwitch.verhit[i] == null ? 0 : (FloorSwitch.vervel[i] < 0 ? -1 : 1);
                FloorSwitch.vervel[i] = FloorSwitch.verhit[i] != null ? 0 : FloorSwitch.vervel[i];
            }
        }
        FloorSwitch.movphy = movphy;
        function timphy() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                FloorSwitch.horextvel[i] = (FloorSwitch.horextvel[i] + FloorSwitch.horvel[i]) * (Game.SceneFreezer.stoped ? 0 : 1);
                FloorSwitch.verextvel[i] = (FloorSwitch.verextvel[i] + FloorSwitch.vervel[i]) * (Game.SceneFreezer.stoped ? 0 : 1);
                FloorSwitch.ext[i] = FloorSwitch.box[i].getExtrapolation(Game.SceneMap.instance.boxesSolids, FloorSwitch.horextvel[i], FloorSwitch.verextvel[i], true);
                FloorSwitch.horextvel[i] = 0;
                FloorSwitch.verextvel[i] = 0;
            }
        }
        FloorSwitch.timphy = timphy;
        function draphy() {
            for (var i = 0; i < FloorSwitch.cou; i++)
                FloorSwitch.box[i].renderAt(FloorSwitch.ext[i].x, FloorSwitch.ext[i].y);
        }
        FloorSwitch.draphy = draphy;
    })(FloorSwitch = Game.FloorSwitch || (Game.FloorSwitch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (FloorSwitch) {
        ...
        FloorSwitch.respla = respla;
        function movpla() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                if (FloorSwitch.verhitdir[i] == FloorSwitch.gradir[i]) {
                    for (var _i = 0, _a = FloorSwitch.verhit[i]; _i < _a.length; _i++) {
                        var hit = _a[_i];
                        var nexpla = null;
                        if (hit.other.data instanceof Game.Platform) {
                            nexpla = hit.other.data;
                    ...
                    pla[i] = nexpla;
                    if (pla[i] != null) {
                        pla[i].chi.push(FloorSwitch.box[i]);
                    }
                }
                else
                    pla[i] = null;
        ...
        FloorSwitch.movpla = movpla;
        function timpla() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                if (pla[i] != null) {
                    FloorSwitch.horextvel[i] += pla[i].xvelmov() / Engine.Box.UNIT;
                    FloorSwitch.horextvel[i] += pla[i].yvelmov() / Engine.Box.UNIT;
                }
            }
        }
        FloorSwitch.timpla = timpla;
    })(FloorSwitch = Game.FloorSwitch || (Game.FloorSwitch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (FloorSwitch) {
        Game.addAction("preconfigure", function () {
            FloorSwitch.fra = Game.FrameSelector.complex("flopat", Game.Resources.texgam, 176, 991);
        });
        FloorSwitch.spr = [];
        function conspr(i) {
            FloorSwitch.spr[i] = new Engine.Sprite();
            FloorSwitch.spr[i].enabled = true;
            FloorSwitch.spr[i].yMirror = FloorSwitch.def[i].flip.y;
        }
        FloorSwitch.conspr = conspr;
        function resspr() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                FloorSwitch.spr[i].x = FloorSwitch.def[i].instance.x;
                FloorSwitch.spr[i].y = FloorSwitch.def[i].instance.y;
            }
        }
        FloorSwitch.resspr = resspr;
        function timspr() {
            for (var i = 0; i < FloorSwitch.cou; i++) {
                FloorSwitch.spr[i].xMirror = FloorSwitch.horvel[i] == 0 ? FloorSwitch.spr[i].xMirror : FloorSwitch.horvel[i] < 0;
                FloorSwitch.spr[i].x = FloorSwitch.ext[i].x;
                FloorSwitch.spr[i].y = FloorSwitch.ext[i].y;
            }
        }
        FloorSwitch.timspr = timspr;
        function draspr() {
            for (var i = 0; i < FloorSwitch.cou; i++)
                FloorSwitch.spr[i].render();
        }
        FloorSwitch.draspr = draspr;
    })(FloorSwitch = Game.FloorSwitch || (Game.FloorSwitch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (Fly) {
        ...
        var Instance = /** @class */ (function (_super) {
            ...
            Instance.prototype.onSetFrame = function (_, __, fra) {
                Fly.fraani(this.ind, fra);
                Fly.fraphy(this.ind, fra);
            };
            return Instance;
        }(Game.Entity));
        Fly.Instance = Instance;
        function onReset() {
            Fly.resphy();
            Fly.resspr();
            Fly.resani();
...
(function (Game) {
    ...
    (function (Fly) {
        Fly.ani = [];
        Game.addAction("configure", function () {
            var fra = Game.FrameSelector.complex("fly", Game.Resources.texgam, 56, 967);
            Fly.anispr = new Utils.Animation("ani", true, fra, 7, [0, 1, 2, 1], null);
        });
        function conani(i) {
            Fly.ani[i] = new Utils.Animator();
            Fly.ani[i].owner = Fly.ani[i].listener = Fly.ins[i];
        }
        Fly.conani = conani;
        function resani() {
            for (var i = 0; i < Fly.cou; i++)
                Fly.ani[i].setAnimation(Fly.anispr);
        }
        Fly.resani = resani;
        function fraani(i, fra) {
...
(function (Game) {
    ...
    (function (Fly) {
        ...
        function conphy(i) {
            ...
            box[i].red = 1;
            box[i].green = box[i].blue = 0;
            Game.SceneMap.instance.boxesEnemies.push(box[i]);
            horare[i] = Fly.ins[i].getProperty("horare") * Game.SceneMap.instance.xSizeTile;
            horste[i] = Fly.ins[i].getProperty("horste");
            horsta[i] = Fly.ins[i].getProperty("horsta");
            verare[i] = Fly.ins[i].getProperty("verare") * Game.SceneMap.instance.xSizeTile;
            verste[i] = Fly.ins[i].getProperty("verste");
            versta[i] = Fly.ins[i].getProperty("versta");
        }
        Fly.conphy = conphy;
        function resphy() {
            for (var i = 0; i < Fly.cou; i++) {
...
namespace Game{
    ...
    export class Fly extends Entity implements Utils.AnimatorListener{
        public box:Engine.Box;
        active:boolean;
        ...
        protected constructor(def:any){
            super(def);
            this.box=new Engine.Box();
            box[i].enabled=true;
            box[i].renderable=true;
            FRAMES[0].applyToBox(this.box);
            box[i].data=this;

            SceneMap.instance.boxesEnemies.push(this.box);
        ...
        onSetFrame(_animator: Utils.Animator, _animation: Utils.Animation, frame: Utils.AnimationFrame): void {
            frame.applyToSprite(this.sprite);
        }

        public onDrawObjects(){
            this.sprite.render();
            box[i].render();
        }
...
(function (Game) {
    ...
    (function (Fly) {
        ...
        function conspr(i) {
            ...
            Fly.spr[i].yMirror = Fly.def[i].flip.y;
        }
        Fly.conspr = conspr;
        function resspr() {
            for (var i = 0; i < Fly.cou; i++) {
...
(function (Game) {
    ...
    (function (InvisibleBlock) {
        InvisibleBlock.preserved = false;
        InvisibleBlock.ins = [];
        InvisibleBlock.def = [];
        InvisibleBlock.cou = 0;
        var Instance = /** @class */ (function (_super) {
            __extends(Instance, _super);
            function Instance(_df) {
                var _this = _super.call(this, _df) || this;
                InvisibleBlock.ins[InvisibleBlock.cou] = _this;
                InvisibleBlock.def[InvisibleBlock.cou] = _df;
                InvisibleBlock.def[InvisibleBlock.cou].instance.y -= Game.SceneMap.instance.ySizeTile * 1;
                InvisibleBlock.conbox(InvisibleBlock.cou);
                if (InvisibleBlock.cou == 0)
                    Engine.System.addListenersFrom(Game.Block);
                _this.ind = InvisibleBlock.cou++;
                return _this;
            }
            return Instance;
        }(Game.Entity));
        InvisibleBlock.Instance = Instance;
        function onDrawObjects() {
            InvisibleBlock.drabox();
        }
        InvisibleBlock.onDrawObjects = onDrawObjects;
        function onClearScene() {
            InvisibleBlock.cou = 0;
        }
        InvisibleBlock.onClearScene = onClearScene;
    })(InvisibleBlock = Game.InvisibleBlock || (Game.InvisibleBlock = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var InvisibleBlock;
    (function (InvisibleBlock) {
        InvisibleBlock.box = [];
        function conbox(i) {
            InvisibleBlock.box[i] = new Engine.Box();
            InvisibleBlock.box[i].enabled = InvisibleBlock.box[i].renderable = true;
            InvisibleBlock.box[i].x = InvisibleBlock.def[i].instance.x;
            InvisibleBlock.box[i].y = InvisibleBlock.def[i].instance.y;
            InvisibleBlock.box[i].xSize = Game.SceneMap.instance.xSizeTile;
            InvisibleBlock.box[i].ySize = Game.SceneMap.instance.ySizeTile;
            InvisibleBlock.box[i].data = i;
            Game.SceneMap.instance.boxesSolids.push(InvisibleBlock.box[i]);
        }
        InvisibleBlock.conbox = conbox;
        function drabox() {
            for (var i = 0; i < InvisibleBlock.cou; i++)
                InvisibleBlock.box[i].render();
        }
        InvisibleBlock.drabox = drabox;
    })(InvisibleBlock = Game.InvisibleBlock || (Game.InvisibleBlock = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
...
(function (Game) {
    ...
    var spr = new Engine.Sprite;
    spr.enabled = spr.pinned = true;
    Game.addAction("init", function () { Game.FrameSelector.complex("titlab", Game.Resources.texgam, 716, 202, null, 0, false)[0].applyToSprite(spr); });
...
(function (Game) {
    ...
    (function (Spike) {
        ...
        Spike.fra = [];
        Game.addAction("configure", function () {
            Spike.fra[0] = Game.FrameSelector.complex("spi000", Game.Resources.texgam, 152, 1008);
            Spike.anispr[0] = new Utils.Animation("ani", true, Spike.fra[0], 8, [1, 2, 3, 2], null);
            Spike.fra[1] = Game.FrameSelector.complex("spi090", Game.Resources.texgam, 204, 1008);
            Spike.anispr[1] = new Utils.Animation("ani", true, Spike.fra[1], 8, [1, 2, 3, 2], null);
            Spike.fra[2] = Game.FrameSelector.complex("spi180", Game.Resources.texgam, 256, 1008);
            Spike.anispr[2] = new Utils.Animation("ani", true, Spike.fra[2], 8, [1, 2, 3, 2], null);
            Spike.fra[3] = Game.FrameSelector.complex("spi270", Game.Resources.texgam, 308, 1008);
...
(function (Game) {
    ...
    (function (Switch) {
        Switch.typ = 0;
    })(Switch = Game.Switch || (Game.Switch = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var WallPatroller;
    (function (WallPatroller) {
        WallPatroller.preserved = false;
        WallPatroller.ins = [];
        WallPatroller.def = [];
        WallPatroller.cou = 0;
        function onClearScene() { WallPatroller.cou = 0; }
        WallPatroller.onClearScene = onClearScene;
        var Instance = /** @class */ (function (_super) {
            __extends(Instance, _super);
            function Instance(_df) {
                var _this = _super.call(this, _df) || this;
                WallPatroller.ins[WallPatroller.cou] = _this;
                WallPatroller.def[WallPatroller.cou] = _df;
                WallPatroller.def[WallPatroller.cou].instance.x += Game.SceneMap.instance.xSizeTile * (WallPatroller.def[WallPatroller.cou].flip.x ? 0 : 1);
                WallPatroller.def[WallPatroller.cou].instance.x += WallPatroller.fra[0].xSizeBox * (WallPatroller.def[WallPatroller.cou].flip.x ? 1 : 0);
                WallPatroller.def[WallPatroller.cou].instance.y -= Game.SceneMap.instance.xSizeTile * 0.5;
                WallPatroller.conphy(WallPatroller.cou);
                WallPatroller.conspr(WallPatroller.cou);
                WallPatroller.conani(WallPatroller.cou);
                WallPatroller.conove(WallPatroller.cou);
                WallPatroller.consta(WallPatroller.cou);
                if (WallPatroller.cou == 0)
                    Engine.System.addListenersFrom(Game.WallPatroller);
                _this.ind = WallPatroller.cou++;
                return _this;
            }
            Instance.prototype.onSetFrame = function (_, __, fra) {
                WallPatroller.fraani(this.ind, fra);
                WallPatroller.fraove(this.ind, fra);
            };
            return Instance;
        }(Game.Entity));
        WallPatroller.Instance = Instance;
        function onReset() {
            WallPatroller.resphy();
            WallPatroller.resspr();
            WallPatroller.resove();
            WallPatroller.respla();
            WallPatroller.ressta();
        }
        WallPatroller.onReset = onReset;
        function onMoveUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
            WallPatroller.movphy();
            WallPatroller.movpla();
            WallPatroller.movove();
        }
        WallPatroller.onMoveUpdate = onMoveUpdate;
        function onStepUpdate() {
            if (Game.SceneFreezer.stoped)
                return;
            WallPatroller.stesta();
            WallPatroller.linsta();
        }
        WallPatroller.onStepUpdate = onStepUpdate;
        function onTimeUpdate() {
            WallPatroller.timpla();
            WallPatroller.timphy();
            WallPatroller.timspr();
            WallPatroller.timove();
            WallPatroller.timsta();
        }
        WallPatroller.onTimeUpdate = onTimeUpdate;
        function onDrawObjects() {
            WallPatroller.draspr();
            WallPatroller.draphy();
            WallPatroller.draove();
            WallPatroller.drasta();
        }
        WallPatroller.onDrawObjects = onDrawObjects;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var WallPatroller;
    (function (WallPatroller) {
        WallPatroller.sta = [];
        function consta(i) {
            WallPatroller.conwal(i);
        }
        WallPatroller.consta = consta;
        function ressta() {
            WallPatroller.reswal();
        }
        WallPatroller.ressta = ressta;
        function stesta() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                switch (WallPatroller.sta[i]) {
                    case WallPatroller.stawal:
                        WallPatroller.stewal(i);
                        break;
                }
            }
        }
        WallPatroller.stesta = stesta;
        function linsta() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                switch (WallPatroller.sta[i]) {
                    case WallPatroller.stawal:
                        WallPatroller.linwal(i);
                        break;
                    case WallPatroller.stafal:
                        WallPatroller.linfal(i);
                        break;
                }
            }
        }
        WallPatroller.linsta = linsta;
        function timsta() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                switch (WallPatroller.sta[i]) {
                    case WallPatroller.stawal:
                        WallPatroller.timwal(i);
                        break;
                }
            }
        }
        WallPatroller.timsta = timsta;
        function drasta() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                switch (WallPatroller.sta[i]) {
                    case WallPatroller.stawal:
                        WallPatroller.drawal(i);
                        break;
                }
            }
        }
        WallPatroller.drasta = drasta;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (WallPatroller) {
        WallPatroller.ani = [];
        function conani(i) {
            WallPatroller.ani[i] = new Utils.Animator();
            WallPatroller.ani[i].owner = WallPatroller.ani[i].listener = WallPatroller.ins[i];
        }
        WallPatroller.conani = conani;
        function fraani(i, fra) {
            fra.applyToSprite(WallPatroller.spr[i]);
            WallPatroller.spr[i].xOffset += WallPatroller.spr[i].xMirror ? WallPatroller.box[i].xSize : 0;
        }
        WallPatroller.fraani = fraani;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (WallPatroller) {
        ...
        WallPatroller.conove = conove;
        function resove() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                ove[i].x = WallPatroller.box[i].x;
                ove[i].y = WallPatroller.box[i].y;
            }
        }
        WallPatroller.resove = resove;
        function movove() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                ove[i].x = WallPatroller.box[i].x;
                ove[i].y = WallPatroller.box[i].y;
        ...
        WallPatroller.movove = movove;
        function fraove(i, fra) {
            fra.applyToBox(ove[i]);
            ove[i].xOffset = WallPatroller.spr[i].xMirror ? -WallPatroller.box[i].xSize : ove[i].xOffset;
        }
        WallPatroller.fraove = fraove;
        function timove() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                ove[i].x = WallPatroller.ext[i].x;
                ove[i].y = WallPatroller.ext[i].y;
            }
        }
        WallPatroller.timove = timove;
        function draove() {
            for (var i = 0; i < WallPatroller.cou; i++)
                ove[i].render();
        }
        WallPatroller.draove = draove;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (WallPatroller) {
        WallPatroller.box = [];
        WallPatroller.horvel = [];
        WallPatroller.vervel = [];
        var maxhorvel = 3;
        var gra = 0.1;
        WallPatroller.grasca = [];
        WallPatroller.gradir = [];
        WallPatroller.horhit = [];
        WallPatroller.verhit = [];
        WallPatroller.horhitdir = [];
        WallPatroller.verhitdir = [];
        WallPatroller.ext = [];
        WallPatroller.horextvel = [];
        WallPatroller.verextvel = [];
        function conphy(i) {
            WallPatroller.box[i] = new Engine.Box();
            WallPatroller.box[i].enabled = WallPatroller.box[i].renderable = true;
            WallPatroller.fra[0].applyToBox(WallPatroller.box[i]);
            Game.Platform.sol.push(WallPatroller.box[i]);
        }
        WallPatroller.conphy = conphy;
        function resphy() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                WallPatroller.box[i].enabled = true;
                WallPatroller.box[i].x = WallPatroller.def[i].instance.x;
                WallPatroller.box[i].y = WallPatroller.def[i].instance.y;
                WallPatroller.horvel[i] = 0;
                WallPatroller.vervel[i] = 0;
                WallPatroller.grasca[i] = 1;
                WallPatroller.gradir[i] = WallPatroller.def[i].flip.x ? -1 : 1;
                WallPatroller.horhit[i] = null;
                WallPatroller.verhit[i] = null;
                WallPatroller.horhitdir[i] = 0;
                WallPatroller.verhitdir[i] = 0;
                WallPatroller.horextvel[i] = 0;
                WallPatroller.verextvel[i] = 0;
            }
        }
        WallPatroller.resphy = resphy;
        function movphy() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                WallPatroller.verhit[i] = WallPatroller.box[i].cast(Game.SceneMap.instance.boxesSolids, null, false, WallPatroller.vervel[i], true, Engine.Box.LAYER_ALL);
                WallPatroller.box[i].translate(WallPatroller.verhit[i], false, WallPatroller.vervel[i], true);
                WallPatroller.verhitdir[i] = WallPatroller.verhit[i] == null ? 0 : (WallPatroller.vervel[i] < 0 ? -1 : 1);
                //vervel[i]=verhit[i]!=null?0:vervel[i];
                WallPatroller.horvel[i] += gra * WallPatroller.grasca[i] * WallPatroller.gradir[i];
                WallPatroller.horvel[i] = Math.abs(WallPatroller.horvel[i]) > maxhorvel ? Game.dir(WallPatroller.horvel[i]) * maxhorvel : WallPatroller.horvel[i];
                WallPatroller.horhit[i] = WallPatroller.box[i].cast(Game.SceneMap.instance.boxesSolids, null, true, WallPatroller.horvel[i] == 0 ? WallPatroller.gradir[i] : WallPatroller.horvel[i], WallPatroller.horvel[i] != 0, Engine.Box.LAYER_ALL);
                WallPatroller.box[i].translate(WallPatroller.horhit[i], true, WallPatroller.horvel[i], true);
                WallPatroller.horhitdir[i] = WallPatroller.horhit[i] == null ? 0 : (WallPatroller.horvel[i] < 0 ? -1 : 1);
                WallPatroller.horvel[i] = WallPatroller.horhit[i] != null ? 0 : WallPatroller.horvel[i];
            }
        }
        WallPatroller.movphy = movphy;
        function timphy() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                WallPatroller.horextvel[i] = (WallPatroller.horextvel[i] + WallPatroller.horvel[i]) * (Game.SceneFreezer.stoped ? 0 : 1);
                WallPatroller.verextvel[i] = (WallPatroller.verextvel[i] + WallPatroller.vervel[i]) * (Game.SceneFreezer.stoped ? 0 : 1);
                WallPatroller.ext[i] = WallPatroller.box[i].getExtrapolation(Game.SceneMap.instance.boxesSolids, WallPatroller.horextvel[i], WallPatroller.verextvel[i], true);
                WallPatroller.horextvel[i] = 0;
                WallPatroller.verextvel[i] = 0;
            }
        }
        WallPatroller.timphy = timphy;
        function draphy() {
            for (var i = 0; i < WallPatroller.cou; i++)
                WallPatroller.box[i].renderAt(WallPatroller.ext[i].x, WallPatroller.ext[i].y);
        }
        WallPatroller.draphy = draphy;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (WallPatroller) {
        ...
        WallPatroller.respla = respla;
        function movpla() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                if (WallPatroller.horhitdir[i] == WallPatroller.gradir[i]) {
                    for (var _i = 0, _a = WallPatroller.horhit[i]; _i < _a.length; _i++) {
                        var hit = _a[_i];
                        var nexpla = null;
                        if (hit.other.data instanceof Game.Platform) {
                            nexpla = hit.other.data;
                    ...
                    pla[i] = nexpla;
                    if (pla[i] != null) {
                        pla[i].chi.push(WallPatroller.box[i]);
                    }
                }
                else
                    pla[i] = null;
        ...
        WallPatroller.movpla = movpla;
        function timpla() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                if (pla[i] != null) {
                    WallPatroller.horextvel[i] += pla[i].xvelmov() / Engine.Box.UNIT;
                    WallPatroller.verextvel[i] += pla[i].yvelmov() / Engine.Box.UNIT;
                }
            }
        }
        WallPatroller.timpla = timpla;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (WallPatroller) {
        Game.addAction("preconfigure", function () {
            WallPatroller.fra = Game.FrameSelector.complex("walpat", Game.Resources.texgam, 56, 947);
        });
        WallPatroller.spr = [];
        function conspr(i) {
            WallPatroller.spr[i] = new Engine.Sprite();
            WallPatroller.spr[i].enabled = true;
            WallPatroller.spr[i].xMirror = WallPatroller.def[i].flip.x;
        }
        WallPatroller.conspr = conspr;
        function resspr() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                WallPatroller.spr[i].x = WallPatroller.def[i].instance.x;
                WallPatroller.spr[i].y = WallPatroller.def[i].instance.y;
            }
        }
        WallPatroller.resspr = resspr;
        function timspr() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                WallPatroller.spr[i].yMirror = WallPatroller.vervel[i] == 0 ? WallPatroller.spr[i].yMirror : WallPatroller.vervel[i] > 0;
                WallPatroller.spr[i].x = WallPatroller.ext[i].x;
                WallPatroller.spr[i].y = WallPatroller.ext[i].y;
            }
        }
        WallPatroller.timspr = timspr;
        function draspr() {
            for (var i = 0; i < WallPatroller.cou; i++)
                WallPatroller.spr[i].render();
        }
        WallPatroller.draspr = draspr;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    var WallPatroller;
    (function (WallPatroller) {
        WallPatroller.stacou = WallPatroller.stacou || 0;
        WallPatroller.stafal = WallPatroller.stacou++;
        var anifal;
        Game.addAction("configure", function () { anifal = new Utils.Animation("fal", false, WallPatroller.fra, 5, [1, 2, 3, 2], null); });
        function setfal(i) {
            WallPatroller.ani[i].setAnimation(anifal);
            WallPatroller.sta[i] = WallPatroller.stafal;
        }
        WallPatroller.setfal = setfal;
        function linfal(i) {
            if (WallPatroller.horhitdir[i] == WallPatroller.gradir[i])
                WallPatroller.setwal(i);
        }
        WallPatroller.linfal = linfal;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (WallPatroller) {
        WallPatroller.stacou = WallPatroller.stacou || 0;
        WallPatroller.stawal = WallPatroller.stacou++;
        var aniwal;
        Game.addAction("configure", function () { aniwal = new Utils.Animation("wal", true, WallPatroller.fra, 7, [1, 2, 3, 2], null); });
        var vel = [];
        var patbox = [];
        var fliaby = [];
        function conwal(i) {
            vel[i] = WallPatroller.ins[i].getProperty("vel");
            fliaby[i] = WallPatroller.ins[i].getProperty("fliaby");
            patbox[i] = new Engine.Box();
            patbox[i].enabled = patbox[i].renderable = true;
            patbox[i].xSize = patbox[i].ySize = 1;
            patbox[i].xOffset = -(WallPatroller.fra[0].xSizeBox + 1) * (WallPatroller.def[i].flip.x ? 1 : 0);
            patbox[i].yOffset = -0.5;
        }
        WallPatroller.conwal = conwal;
        function reswal() {
            for (var i = 0; i < WallPatroller.cou; i++) {
                setwal(i);
            }
        }
        WallPatroller.reswal = reswal;
        function setwal(i) {
            WallPatroller.ani[i].setAnimation(aniwal);
            WallPatroller.vervel[i] = vel[i] * (WallPatroller.def[i].flip.y ? 1 : -1);
            WallPatroller.sta[i] = WallPatroller.stawal;
        }
        WallPatroller.setwal = setwal;
        function stewal(i) {
            if (WallPatroller.verhitdir[i] != 0 || (WallPatroller.horhitdir[i] == WallPatroller.gradir[i] && fliaby[i] && cheaby(i))) {
                WallPatroller.spr[i].yMirror = !WallPatroller.spr[i].yMirror;
                WallPatroller.vervel[i] *= -1;
            }
        }
        WallPatroller.stewal = stewal;
        function cheaby(i) {
            patbox[i].x = WallPatroller.box[i].x;
            patbox[i].y = WallPatroller.box[i].y;
            return patbox[i].collide(Game.SceneMap.instance.boxesSolids) == null;
        }
        function linwal(i) {
            if (WallPatroller.horhitdir[i] == 0)
                WallPatroller.setfal(i);
        }
        WallPatroller.linwal = linwal;
        function timwal(i) {
            patbox[i].x = WallPatroller.ext[i].x;
            patbox[i].y = WallPatroller.ext[i].y;
        }
        WallPatroller.timwal = timwal;
        function drawal(i) {
            patbox[i].render();
        }
        WallPatroller.drawal = drawal;
    })(WallPatroller = Game.WallPatroller || (Game.WallPatroller = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
...
(function (Game) {
    ...
    (function (Goal) {
        Goal.ani = [];
        Game.addAction("configure", function () {
            Goal.fra = Game.FrameSelector.complex("coi", Game.Resources.texgam, 0, 1008);
...
(function (Game) {
    ...
    (function (Goal) {
        ...
        Goal.ovegot = ovegot;
        //for(var i=0;i<20;i++)console.error("DELETE SKIP LEVEL BUTTON "+Math.random());
        function stegot() {
...
(function (Game) {
    ...
    (function (Goal) {
        ...
        var FRAMES_GOAL;
        Game.addAction("configure", function () {
            FRAMES_GOAL = Game.FrameSelector.complex("goal particles", Game.Resources.texgam, 360, 1008);
        });
    ...
    (function (titlab) {
        ...
        function mak(y) {
            if (Game.plu) {
                ...
                var texver = new Utils.Text();
                texver.font = Game.FontManager.a;
                texver.scale = 1;
                texver.enabled = true;
                texver.pinned = true;
                texver.str = " ";
                texver.xAlignBounds = Utils.AnchorAlignment.MIDDLE;
                texver.xAlignView = Utils.AnchorAlignment.MIDDLE;
                texver.yAlignBounds = Utils.AnchorAlignment.START;
                texver.yAlignView = Utils.AnchorAlignment.MIDDLE;
                texver.xAligned = 30 + 2 + 2 + 1;
                texver.yAligned = -60 + 6 + 6 + 0 + 6 + 12 - 6 + 1;
            }
        }
        titlab.mak = mak;
        var TitleLabel = /** @class */ (function (_super) {
...
(function (Game) {
    ...
    (function (pla) {
        ...
        pla.Instance = Instance;
        function con(def) {
            pla.def = def;
            def.instance.y -= (Game.SceneMap.instance.ySizeTile - pla.fra[0].ySizeBox) * (def.flip.y ? 1 : 0);
            pla.conpau();
            pla.conani();
            pla.concam();
            pla.conphy();
            ...
            pla.DashControl.condes();
            pla.DashControl.contou();
        }
        pla.con = con;
        function onReset() {
            pla.reslos();
...
(function (Game) {
    ...
    var FRAMES;
    Game.addAction("init", function () {
        FRAMES = Game.FrameSelector.complex("player sprites", Game.Resources.texgam, 49, 895);
...
(function (Game) {
    ...
    (function (pla) {
        ...
        function onSetFrame(_, __, fra) {
            ...
            pla.spr.yOffset += pla.spr.yMirror ? pla.box.ySize : 0;
        }
...
(function (Game) {
    ...
    (function (pla) {
        function confli() {
            pla.toppar = new pla.ArrowParticles();
            pla.botpar = new pla.ArrowParticles();
        }
        pla.confli = confli;
        function stefli() {
            if (Game.pla.DashControl.cou > 0 && pla.box.enabled) {
                // if(xhit!=null){
                //     for(var hit of xhit){
                //         var ind=hit.other.data;
                //         if(ind!=null&&chehorhit(ind)){
                //             Game.Player.DashControl.cou=0;
                //             return;
                //         }
                //     }
                // }
                if (pla.yhit != null && pla.yhitdir == pla.gradir) {
                    for (var _i = 0, yhit_1 = pla.yhit; _i < yhit_1.length; _i++) {
                        var hit = yhit_1[_i];
                        var ind = hit.other.data;
                        if (ind != null && cheverhit(ind)) {
                            Game.pla.DashControl.cou = 0;
                            return;
                        }
                    }
                }
        ...
        pla.stefli = stefli;
        // function chehorhit(i:number){
        //     if(Arrow.hor[i]){
        //         if(xhitdir==Arrow.dir[i]){
        //             box.x+=xhitdir*box.xSize;
        //             var hit=box.collide(SceneMap.instance.boxesSolids);
        //             while(hit!=null){
        //                 box.x+=xhitdir*SceneMap.instance.xSizeTile;
        //                 hit=box.collide(SceneMap.instance.boxesSolids);
        //             }
        //             xhitdir*=0;
        //             //xhitdir*=-1;
        //             //xvel*=-1;
        //             //spr.xMirror=!spr.xMirror;
        //             ani.setCurrentFrame();
        //             return true;
        //         }
        //     }
        ...
        function triflipar() {
            if (pla.gradir < 0)
                pla.botpar.trigger(0, true, true, pla.box.x, pla.box.y - 2.0);
            else
                pla.toppar.trigger(0, true, false, pla.box.x, pla.box.y - 4.0);
        }
        function trigrapar() {
            if (pla.gradir < 0)
                pla.botpar.trigger(2, true, true, pla.box.x, pla.box.y - 2.0 - 2.0);
            else
                pla.toppar.trigger(1, true, false, pla.box.x, pla.box.y - 4.0 + 2.0);
        }
        function cheverhit(i) {
            if (!Game.Arrow.hor[i]) {
                if (pla.gradir == Game.Arrow.dir[i]) {
                    triflipar();
                    pla.box.y += pla.gradir * pla.box.ySize;
                    var hit = pla.box.collide(Game.SceneMap.instance.boxesSolids);
                    while (hit != null) {
                        pla.box.y += pla.gradir * Game.SceneMap.instance.ySizeTile;
                        hit = pla.box.collide(Game.SceneMap.instance.boxesSolids);
                    }
                    pla.yhitdir *= -1;
                    pla.gradir *= -1;
                    pla.spr.yMirror = !pla.spr.yMirror;
                    pla.ani.setCurrentFrame();
                    Game.Resources.audfli.play();
                    return true;
                }
                else {
                    trigrapar();
...
(function (Game) {
    ...
    (function (pla) {
        ...
        var ANIMS;
        Game.addAction("configure", function () {
            FRAMES = Game.FrameSelector.complex("particles", Game.Resources.texgam, 393, 1008);
            ANIMS = [];
            ANIMS[0] = new Utils.Animation("anim pla", false, FRAMES, 1, [0, 0, 0, 0, 0, 1, 1, 2, 2, 3], null);
            ANIMS[1] = new Utils.Animation("anim pink", false, FRAMES, 1, [4, 4, 4, 4, 4, 5, 5, 6, 6, 7], null);
            ANIMS[2] = new Utils.Animation("anim brown ", false, FRAMES, 1, [8, 8, 8, 8, 8, 9, 9, 10, 10, 11], null);
        });
        var SPEED = 60;
        var ArrowParticles = /** @class */ (function () {
            ...
            ArrowParticles.prototype.onReset = function () {
                this.spriteA.enabled = false;
                this.spriteB.enabled = false;
                this.spriteC.enabled = false;
                this.animator.enabled = false;
            };
            ArrowParticles.prototype.trigger = function (color, horizontal, flipped, x, y) {
                ...
                this.animator.setAnimation(ANIMS[color]);
            };
            ArrowParticles.prototype.onSetFrame = function (_animator, _animation, _frame) {
                _frame.applyToSprite(this.spriteA);
                _frame.applyToSprite(this.spriteB);
                _frame.applyToSprite(this.spriteC);
            };
            ArrowParticles.prototype.onTimeUpdate = function () {
            ...
            ArrowParticles.prototype.dra = function () {
        ...
        pla.ArrowParticles = ArrowParticles;
    })(pla = Game.pla || (Game.pla = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (pla) {
        Game.addAction("preinit", function () {
            pla.fra = Game.FrameSelector.complex("Player", Game.Resources.texgam, 0, 991);
        });
...
(function (Game) {
    ...
    (function (pla) {
        ...
        var FRAMES_GOAL;
        Game.addAction("configure", function () {
            FRAMES_GOAL = Game.FrameSelector.complex("dead particles", Game.Resources.texgam, 360, 1008);
        });
...
(function (Game) {
    ...
    (function (pla) {
        pla.movvel = 0.8;
        function resmov() {
            pla.xvel = pla.movvel * (pla.def.flip.x ? -1 : 1);
        }
        pla.resmov = resmov;
    })(pla = Game.pla || (Game.pla = {}));
})(Game || (Game = {}));
...
(function (Game) {
    ...
    (function (pla) {
        ...
        pla.conpau = conpau;
        function stepau() {
            if (pau != (Game.SceneFreezer.stoped || (Game.Level.practice && Game.Level.practiceFinished))) {
                pau = Game.SceneFreezer.stoped || (Game.Level.practice && Game.Level.practiceFinished);
                if (pau)
                    Game.triggerActions("gameplayStop");
                else
                    Game.triggerActions("gameplayStart");
            }
        }
        pla.stepau = stepau;
    })(pla = Game.pla || (Game.pla = {}));
...
(function (Game) {
    ...
    (function (pla) {
        ...
        pla.resphy = resphy;
        function movphy() {
            pla.xhit = pla.box.cast(Game.SceneMap.instance.boxesSolids, null, true, pla.xvel, true, Engine.Box.LAYER_ALL);
            pla.box.translate(pla.xhit, true, pla.xvel, true);
            pla.xhitdir = pla.xhit == null ? 0 : (pla.xvel < 0 ? -1 : 1);
            //xvel=xhit!=null?0:xvel;
            pla.yvel += pla.gra * pla.grasca * pla.gradir;
            pla.yvel = Math.abs(pla.yvel) > pla.maxvervel ? Game.dir(pla.yvel) * pla.maxvervel : pla.yvel;
            pla.yhit = pla.box.cast(Game.SceneMap.instance.boxesSolids, null, false, pla.yvel == 0 ? pla.gradir : pla.yvel, pla.yvel != 0, Engine.Box.LAYER_ALL);
            pla.box.translate(pla.yhit, false, pla.yvel, true);
            pla.yhitdir = pla.yhit == null ? 0 : (pla.yvel < 0 ? -1 : 1);
            pla.yvel = pla.yhit != null ? 0 : pla.yvel;
        }
        pla.movphy = movphy;
        function timphy() {
            pla.horextvel = (pla.horextvel + pla.xvel) * (Game.SceneFreezer.stoped ? 0 : 1);
            pla.verextvel = (pla.verextvel + pla.yvel) * (Game.SceneFreezer.stoped ? 0 : 1);
...
(function (Game) {
    ...
    (function (pla_1) {
        ...
        pla_1.respla = respla;
        function movpla() {
            if (pla_1.yhitdir == pla_1.gradir) {
                for (var _i = 0, yhit_2 = pla_1.yhit; _i < yhit_2.length; _i++) {
                    var hit = yhit_2[_i];
                    var nexpla = null;
                    if (hit.other.data instanceof Game.Platform) {
                        nexpla = hit.other.data;
...
(function (Game) {
    ...
    (function (pla) {
        ...
        (function (DashControl) {
            DashControl.preserved = false;
            DashControl.ste = 5;
            function condes() {
                DashControl.inp = new Game.Control();
                DashControl.inp.enabled = true;
                DashControl.inp.freezeable = true;
                DashControl.inp.listener = DashControl;
                DashControl.inp.useKeyboard = true;
                DashControl.inp.newInteractionRequired = true;
                DashControl.inp.useMouse = true;
                DashControl.inp.mouseButtons = [2];
                DashControl.inp.keys = [Engine.Keyboard.SPACE, "spacebar", "Spacebar", "SPACEBAR", "space", "Space", " "];
                Engine.System.addListenersFrom(DashControl);
            }
            DashControl.condes = condes;
            function onReset() {
                DashControl.cou = 0;
            }
            DashControl.onReset = onReset;
            function onStepUpdate() {
                if (!Game.SceneFreezer.stoped) {
                    DashControl.cou--;
                    DashControl.cou = DashControl.inp.pressed ? DashControl.ste : DashControl.cou;
                }
            }
            DashControl.onStepUpdate = onStepUpdate;
        })(DashControl = pla.DashControl || (pla.DashControl = {}));
    })(pla = Game.pla || (Game.pla = {}));
})(Game || (Game = {}));
var Game;
(function (Game) {
    ...
    (function (pla) {
        ...
        (function (DashControl) {
            function contou() {
                if (Game.IS_TOUCH) {
                    DashControl.inp.useTouch = true;
                    DashControl.inp.bounds = new Engine.Sprite();
                    DashControl.inp.bounds.enabled = true;
                    DashControl.inp.bounds.pinned = true;
                    DashControl.inp.listener = DashControl;
                }
            }
            DashControl.contou = contou;
            function onViewUpdate() {
                if (Game.IS_TOUCH) {
                    DashControl.inp.bounds.x = -Engine.Renderer.xSizeView * 0.5;
                    DashControl.inp.bounds.y = -Engine.Renderer.ySizeView * 0.5;
                    DashControl.inp.bounds.xSize = Engine.Renderer.xSizeView * 0.5;
                    DashControl.inp.bounds.ySize = Engine.Renderer.ySizeView;
                }
            }
            DashControl.onViewUpdate = onViewUpdate;
        })(DashControl = pla.DashControl || (pla.DashControl = {}));
    })(pla = Game.pla || (Game.pla = {}));
})(Game || (Game = {}));
var Game;
    ...
    (function (pla) {
        ...
        (function (JumpControl) {
            ...
            function condes() {
                ...
                JumpControl.inp.useMouse = true;
                JumpControl.inp.mouseButtons = [0];
                JumpControl.inp.keys = [Engine.Keyboard.C, Engine.Keyboard.UP, "up", "Up"];
...
(function (Game) {
    ...
    (function (pla) {
        ...
        (function (JumpControl) {
            ...
            function onViewUpdate() {
                if (Game.IS_TOUCH) {
                    JumpControl.inp.bounds.y = -Engine.Renderer.ySizeView * 0.5;
                    JumpControl.inp.bounds.xSize = Engine.Renderer.xSizeView * 0.5;
                    JumpControl.inp.bounds.ySize = Engine.Renderer.ySizeView;
                }
            }
...
(function (Game) {
    var pla;
    (function (pla) {
        pla.stacou = pla.stacou || 0;
        pla.staasc = pla.stacou++;
        Game.addAction("init", function () { pla.aniasc = new Utils.Animation("asc", false, pla.fra, 2, [7, 8], null); });
        function entasc() {
            if (Game.dir(pla.yvel) == -pla.gradir) {
                pla.ani.setAnimation(pla.aniasc);
                pla.sta = pla.staasc;
                return true;
            }
            return false;
...
(function (Game) {
    ...
    (function (pla) {
        pla.stacou = pla.stacou || 0;
        pla.stafal = pla.stacou++;
        Game.addAction("init", function () { pla.anifal = new Utils.Animation("fal", false, pla.fra, 2, [9, 10, 11], null); });
        function setfal() {
            pla.ani.setAnimation(pla.anifal);
            pla.sta = pla.stafal;
        }
        pla.setfal = setfal;
        function entfal() {
            if (pla.yhitdir == 0 && Game.dir(pla.yvel) == pla.gradir) {
                setfal();
                return true;
            }
            return false;
...
(function (Game) {
    ...
    (function (pla) {
        ...
        pla.entjum = entjum;
        function stejum() {
            pla.holjum = pla.coujum < minstejum || (pla.holjum && pla.JumpControl.inp.down);
            pla.yvel *= (pla.holjum ? 1 : mulstojum);
            pla.coujum++;
        }
        pla.stejum = stejum;
        function linjum() {
...
(function (Game) {
    ...
    (function (pla) {
        pla.stacou = pla.stacou || 0;
        pla.stawal = pla.stacou++;
        Game.addAction("init", function () { pla.aniwal = new Utils.Animation("wal", true, pla.fra, 7, [5, 6, 5, 4], null); });
        function setwal() {
            pla.ani.setAnimation(pla.aniwal);
            pla.sta = pla.stawal;
        }
        pla.setwal = setwal;
        function entwal() {
            if (pla.yhitdir == pla.gradir) {
                pla.ani.setAnimation(pla.aniwal);
                pla.sta = pla.stawal;
                return true;
            }
