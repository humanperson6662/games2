_speedrunStatsHandler = function () {
	...
	*/
	...
	var onNewSpeedrun = function () {
		...
		speedrun_stats.totalSwordSwings = 0;
	...
	var onSound = function (soundName) {
		switch (soundName) {
			case "jump":
				...
				break;
			case "attackmiss":
				speedrun_stats.totalSwordSwings += 1;
				break;
			case "playerdie":
	...
	var displaySpeedrunStats = function () {
		...
		$("#total_swordswings_stat").text(speedrun_stats.totalSwordSwings);
		...
		for (var level = 1; level <= 15; level++) {
			...
			if (!currentLevelTime) {
				...
				$levelTimeElement.text(currentLevelTime.toFixed(3));
			}

			if (!currentLevelLoadTime || currentLevelLoadTime < 2.99) {
				speedrun_stats.fake = true;
			}
		}
